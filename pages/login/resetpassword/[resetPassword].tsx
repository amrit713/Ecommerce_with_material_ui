...
import { resetPasswordUser } from "../../../store/action/authAction";
...
function resetPassword() {
  ...
  const { passwordReset, loading } = useTypedSelector(
    (state) => state.auth
  );
  const { values, errors, handleBlur, touched, handleChange, handleSubmit } =
    useFormik({
      ...
      onSubmit: (value: any) => {
        const parameter = {
          value,
          resetToken: router.query.resetPassword,
        };
        
        dispatch(resetPasswordUser(parameter));
      },
    });
    ...
    <div className="bg-white my-10  m-auto max-w-[600px] min-w-[310px] rounded-lg box_shadow px-[48px] py-[32px] text-center space-y-6 ">
      ...
      <form action="" className="space-y-6" onSubmit={handleSubmit}>
        ...
        <div className="space-y-2">
          ...
          {passwordReset.error ? (
            <Alert severity="error">{passwordReset.error}</Alert>
          ) : null}

          {passwordReset.message ? (
            <Alert severity="error">{passwordReset.message}</Alert>
          ) : null}
        </div>
