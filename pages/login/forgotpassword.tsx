...
import { forgotPasswordUser } from "../../store/action/authAction";
...
function forgotPassword() {
  ...
  const { forgetPassword, loading } = useTypedSelector((state) => state.auth);
    ...
    <div className="bg-white mt-72  m-auto max-w-[600px] min-w-[310px] rounded-lg box_shadow px-[48px] py-[32px] text-center space-y-6 ">
      ...
      <form action="" className="space-y-6" onSubmit={handleSubmit}>
        <div className="space-y-2">
          ...
          {forgetPassword.error ? (
            <Alert severity="error">{forgetPassword.error}</Alert>
          ) : null}
          {forgetPassword.message ? (
            <Alert severity="error">{forgetPassword.message}</Alert>
          ) : null}
        </div>
      ...
      <Link href="/login">
        <Button className="w-full space-x-4 " type="submit">
          <ArrowBackIcon className="text-md " />
          <span>Login</span>
        </Button>
      </Link>
    </div>
